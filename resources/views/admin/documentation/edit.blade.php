...
@section('content')
    ...
    <div class="content">
        ...
        <div>
            <form method="POST" action="/update-documentation" enctype="multipart/form-data">
                {!! csrf_field() !!}
                <div class="form-group col-sm-12 col-lg-12">
                    {!! Form::label('title', 'Название:') !!}
                    {!! Form::text('title', $documentation->title, ['class' => 'form-control']) !!}
                </div>

                <div style="display: none">
                    {!! Form::label('type', 'Название:') !!}
                    {!! Form::text('type', $documentation->type, ['class' => 'form-control']) !!}
                </div>

                <div>
                    {!! Form::label('id', 'Название:') !!}
                    {!! Form::text('id', $documentation->id, ['class' => 'form-control', 'style' => 'display: none']) !!}
                </div>
                                ...
                                @if($files)
                                    <table  style="width: 100%">
                                        ...
                                        @foreach($files as $file)
                                            <tr>
                                                <input type="text" value="{!! $file->link !!}" style="display: none" id="file-link{!! $file->id !!}">
                                                <td>{!! $file->old_name !!}</td>
                                                <td><button class="text-right" type="button" onclick="add_link('file-link{!! $file->id !!}')">Вставить ссылку</button></td>
                                            </tr>
                                        @endforeach
                                        </tbody>
                                    </table>
                ...
                <div class="form-group col-sm-12 col-lg-12">
                    ...
                    <button class="btn btn-warning" data-toggle="modal" data-target="#exampleModalCenter" type="button" id="plus_file">Выбрать файл</button>
                    <br>
                    {!! Form::label('text', 'Описание:') !!}
                    {!! Form::textarea('text', $documentation->text, ['class' => 'form-control', 'id' => 'text']) !!}
                </div>
                    ...
                    @foreach($products_all as $products)
                        @if($products->is_active == 1)
                            <div style="padding-left: 20px;">
                                {{ $products->name }}
                                @if(\App\Models\DocumentationToProduct::where('id_documentation', '=', $documentation->id)->where('id_product', '=', $products->id)->first())
                                    {!! Form::hidden('product-name[]', $products->id, ['id' => 'product'.$products->id]) !!}
                                    <input type="checkbox" id="one{{ $products->id }}" class="checkbox_carousels"
                                       onchange="checkbox_carousels('#product{!! $products->id !!}', '{!! $products->id !!}', 'one{!! $products->id !!}')" checked>
                                @else
